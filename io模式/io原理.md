...
## 数据为什么要从内核空间拷贝到用户空间
  假设，网卡读到的数据直接返回给用户空间，必然会造成频繁的系统中断，因为网卡读到的数据不一定是完整的，可能是断断续续过来的，所以使用内核作为缓冲区，等待数据完整接受后进行一次系统中断即可
...
## 同步io和异步io
  ...
  - 同步和异步还可以分为阻塞和非阻塞，组合起来一共四种模式，其中的异步io，阻塞io和非阻塞io就是所谓的aio,bio,nio


## 进程的协作关系
- 同步：多个进程中发生的事件存在某种先后顺序
